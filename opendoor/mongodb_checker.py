...
class MongoDBChecker:
    ...
    def __init__(self, tunnel_manager: Optional[SSHTunnelManager] = None):
        ...
        self.collection_unsolicited_name = os.getenv("MONGO_COLLECTION_UNSOLICITED", "unsolicited")
        self._client = None
        self._collection = None
        self._collection_diags = None
        self._collection_unsolicited = None
    ...
    def connect(self) -> tuple[bool, str]:
        ...
        try:
            ...
            self._collection_unsolicited = db[self.collection_unsolicited_name]
    ...
    def check_door_alarm(self, device_id: str, installation_date: Optional[datetime] = None) -> dict:
        ...
        Logica:
        - Cerca documenti con allarme porta aperta (EAM_OUT_ALG_19_VAR_39_calc.value = true)
        - Estrae il timestamp dell'allarme (payload.metrics.TIMESTAMP.value)
        - Confronta con installation_date:
          - Se timestamp >= installation_date → allarme VALIDO
          - Se timestamp < installation_date → allarme IGNORATO (pre-installazione)
        
        Args:
            device_id: Il clientId del dispositivo
            installation_date: Data di installazione del dispositivo (opzionale)
            
        Returns:
            Dict con:
                - device_id: str
                - door_open: bool (True se esiste allarme)
                - door_open_valid: bool (True solo se allarme è successivo a installazione)
                - door_open_timestamp: datetime (timestamp dell'ultimo allarme)
                - error: str (eventuale errore)
        """
        result = {
            "device_id": device_id,
            "door_open": False,
            "door_open_valid": False,
            "door_open_timestamp": None,
            "error": ""
        }
        
        # Verifica connessione - usa collection unsolicited
        if self._collection_unsolicited is None:
            success, msg = self.connect()
            if not success:
                result["error"] = msg
                return result
        ...
        try:
            # Query per trovare l'ultimo allarme porta aperta
            # Cerca documenti con EAM_OUT_ALG_19_VAR_39_calc.value = true
            pipeline = [
                {
                    "$match": {
                        "clientId": device_id,
                        "payload.metrics.EAM_OUT_ALG_19_VAR_39_calc.value": True
                    }
                },
                {
                    "$sort": {"payload.metrics.TIMESTAMP.value": -1}  # Ordina per timestamp decrescente
                },
                {
                    "$limit": 1
                },
                {
                    "$project": {
                        ...
                        "clientId": 1,
                        "timestamp": "$payload.metrics.TIMESTAMP.value",
                        "alarm_value": "$payload.metrics.EAM_OUT_ALG_19_VAR_39_calc.value"
                    }
                }
            ]
            
            docs = list(self._collection_unsolicited.aggregate(pipeline))
            
            if docs:
                # Esiste almeno un allarme porta aperta
                result["door_open"] = True
                
                # Estrai il timestamp dell'allarme
                alarm_ts = docs[0].get("timestamp")
                if alarm_ts:
                    # Gestisci il caso in cui sia un dict con $numberLong
                    if isinstance(alarm_ts, dict) and "$numberLong" in alarm_ts:
                        alarm_ts = int(alarm_ts["$numberLong"])
                    elif isinstance(alarm_ts, (int, float)):
                        alarm_ts = int(alarm_ts)
                    
                    # Converti da millisecondi a datetime
                    alarm_datetime = datetime.fromtimestamp(alarm_ts / 1000.0)
                    result["door_open_timestamp"] = alarm_datetime
                    
                    # Valida l'allarme rispetto alla data di installazione
                    if installation_date:
                        # L'allarme è valido solo se successivo all'installazione
                        if alarm_datetime >= installation_date:
                            result["door_open_valid"] = True
                        else:
                            # Allarme pre-installazione, ignoralo
                            result["door_open_valid"] = False
                    else:
                        # Nessuna data di installazione, consideriamo l'allarme valido
                        # (comportamento conservativo)
                        result["door_open_valid"] = True
            else:
                # Nessun allarme porta aperta trovato
