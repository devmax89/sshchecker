...
class DataLoader:
    ...
    def load_file(self, file_path: Optional[str] = None) -> tuple[bool, str, int]:
        ...
        try:
            ...
            # Verifica se esiste la colonna "Data Installazione Digil" (colonna P)
            data_inst_col = None
            possible_data_names = ['Data Installazione Digil', 'Data installazione Digil',
                                   'DATA INSTALLAZIONE DIGIL', 'Data_Installazione_Digil',
                                   'Data Installazione', 'DataInstallazione']
            
            for col_name in possible_data_names:
                if col_name in self._df.columns:
                    data_inst_col = col_name
                    break
            
            # Se non trovata con il nome, cerca colonne con "data" e "installazione"
            if data_inst_col is None:
                for col in self._df.columns:
                    col_lower = str(col).lower()
                    if 'data' in col_lower and 'installazione' in col_lower and 'digil' in col_lower:
                        data_inst_col = col
                        break
            
            self._data_installazione_col = data_inst_col
            ...
            if data_inst_col:
                msg += f", (colonna '{data_inst_col}' trovata)"
            else:
                msg += ", (colonna 'Data Installazione Digil' non trovata)"
    ...
    def get_devices(self, filter_vendor: Optional[str] = None,
        ...
        for _, row in self._df.iterrows():
            ...
            # Leggi "Data Installazione Digil" se disponibile
            data_installazione = None
            if hasattr(self, '_data_installazione_col') and self._data_installazione_col:
                data_inst_value = row.get(self._data_installazione_col, '')
                if pd.notna(data_inst_value):
                    data_installazione = self._parse_installation_date(data_inst_value)
    ...
    def _parse_installation_date(self, date_value) -> Optional[datetime]:
        """
        Parsa la data di installazione dal valore Excel.
        
        Supporta:
        - Formato stringa DD/MM/YYYY (es: "12/12/2025")
        - Formato datetime Excel
        - Formato stringa YYYY-MM-DD
        ...
        try:
            ...
            if isinstance(date_value, str):
                ...
                # Formato DD/MM/YYYY (es: "12/12/2025")
                if '/' in date_str:
            ...
            # Prova conversione generica pandas
            result = pd.to_datetime(date_value)
            if pd.notna(result):
                return result.to_pydatetime()
                
        except Exception as e:
            # Se fallisce il parsing, ritorna None silenziosamente
            pass
...
class ResultExporter:
    ...
    def _get_note_for_device(self, device) -> str:
        ...
        1. Se malfunction_type == "OK" E tipo_installazione_am == "Inst. Completa"
           -> "Verificare Tiro"
        2. Se door_open == True ma door_open_valid == False
           -> "Allarme porta ignorato (pre-installazione)"
        3. Se c'è connectivity_note (es: "Non raggiungibile (Ping KO, SSH KO)")
           -> Mostra la connectivity_note
        4. Altrimenti mostra eventuali errori tecnici
        """
        ...
        # 2. Allarme porta ignorato (pre-installazione)
        if door_open is True and door_open_valid is False:
            notes.append("Allarme porta ignorato (pre-installazione)")
        
        # 3. Connectivity note (se presente)
        ...
        # 4. Altrimenti, raccogli gli errori tecnici
        errors = []
    ...
    def export_diagnostic_results(self, results: list, 
        ...
        try:
            ...
            for r in results:
                ...
                # Data Installazione
                data_inst = getattr(r, 'data_installazione', None)
                data_inst_str = data_inst.strftime("%d/%m/%Y") if data_inst else "-"
                
                # Porta status - mostra data allarme se presente
                if door_open is True:
                    if door_open_timestamp:
                        # Mostra KO con data allarme
                        porta_status = f"KO - {door_open_timestamp.strftime('%Y-%m-%d %H:%M')}"
                    else:
                        porta_status = "KO"
                ...
                # Genera la nota (con logica "Verificare Tiro" e allarme porta ignorato)
                ...
                data.append({
                    ...
                    "Data Installazione": data_inst_str,
